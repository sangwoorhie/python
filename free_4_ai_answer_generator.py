...
이 답변이 질문에 적절한지 엄격하게 평가해주세요."""
    ...
    # ==================================================
    # 🔥 정확도 개선 패치 - 향상된 유사 답변 검색 메서드
    # ==================================================
    def search_similar_answers_enhanced(self, query: str, top_k: int = 8, similarity_threshold: float = 0.6, lang: str = 'ko') -> list:
        try:
            with memory_cleanup():
                logging.info(f"=== 향상된 검색 시작 ===")
                logging.info(f"원본 질문: {query[:100]}")
                
                # 🔥 1단계: 다양한 검색 전략으로 더 많은 후보 확보
                all_results = []
                ...
                query_vector = self.create_embedding(query_to_embed)
                if query_vector:
                    results_original = index.query(
                        vector=query_vector,
                        top_k=top_k * 2,  # 더 많이 검색
                        include_metadata=True
                    )
                    all_results.extend(results_original['matches'])
                    logging.info(f"원본 질문 검색: {len(results_original['matches'])}개")
                
                # 1-2. 핵심 키워드 추출 후 재검색
                keywords = self.extract_enhanced_keywords(query_to_embed)
                if len(keywords) >= 2:
                    keyword_query = " ".join(keywords[:4])  # 상위 4개 키워드 조합
                    keyword_vector = self.create_embedding(keyword_query)
                    if keyword_vector:
                        results_keywords = index.query(
                            vector=keyword_vector,
                            top_k=top_k,
                            include_metadata=True
                        )
                        all_results.extend(results_keywords['matches'])
                        logging.info(f"키워드 검색 ({keyword_query}): {len(results_keywords['matches'])}개")
                
                # 1-3. 영어 질문인 경우 한국어 번역 후 검색
                if lang == 'en':
                    korean_query = self.translate_text(query_to_embed, 'en', 'ko')
                    korean_vector = self.create_embedding(korean_query)
                    if korean_vector:
                        results_korean = index.query(
                            vector=korean_vector,
                            top_k=top_k,
                            include_metadata=True
                        )
                        all_results.extend(results_korean['matches'])
                        logging.info(f"한국어 번역 검색: {len(results_korean['matches'])}개")
                
                # 🔥 2단계: 중복 제거 및 스코어 기반 정렬
                unique_results = {}
                for match in all_results:
                    match_id = match['id']
                    if match_id not in unique_results or match['score'] > unique_results[match_id]['score']:
                        unique_results[match_id] = match
                
                sorted_results = sorted(unique_results.values(), key=lambda x: x['score'], reverse=True)[:top_k * 2]
                
                # 🔥 3단계: 향상된 관련성 검증 및 점수 재계산
                enhanced_results = []
                for i, match in enumerate(sorted_results):
                    ...
                    # 3-1. 의미적 유사도 추가 계산
                    semantic_score = self.calculate_semantic_similarity(query_to_embed, question, answer)
                    
                    # 3-2. 키워드 매칭 점수 계산
                    keyword_score = self.calculate_keyword_matching_score(query_to_embed, question, answer)
                    
                    # 3-3. 최종 점수 = 벡터 유사도 (70%) + 의미적 유사도 (20%) + 키워드 매칭 (10%)
                    final_score = score * 0.7 + semantic_score * 0.2 + keyword_score * 0.1
                    
                    # 3-4. 관련성 임계값 적용 (적응적 임계값)
                    adaptive_threshold = max(0.3, similarity_threshold - (i * 0.05))  # 순위가 낮을수록 임계값 완화
                    
                    if final_score >= adaptive_threshold or i < 3:  # 상위 3개는 무조건 포함
                        ...
                        logging.info(f"✅ 포함: #{i+1} 최종점수={final_score:.3f} (벡터={score:.3f}, 의미={semantic_score:.3f}, 키워드={keyword_score:.3f})")
                    else:
                        logging.info(f"❌ 제외: #{i+1} 최종점수={final_score:.3f} < 임계값={adaptive_threshold:.3f}")
                
                # 최종 점수로 재정렬
                enhanced_results.sort(key=lambda x: x['score'], reverse=True)
                
                logging.info(f"향상된 검색 완료: {len(enhanced_results)}개의 고품질 답변 선별")
    ...
    # 🔥 향상된 키워드 추출 메서드
    # ==================================================
    def extract_enhanced_keywords(self, text: str) -> list:
        """더 정교한 키워드 추출"""
        
        # 중요 키워드 패턴 정의
        important_patterns = {
            'bible_books': r'(창세기|출애굽기|레위기|민수기|신명기|여호수아|사사기|룻기|사무엘|열왕기|역대|에스라|느헤미야|에스더|욥기|시편|잠언|전도서|아가|이사야|예레미야|예레미야애가|에스겔|다니엘|호세아|요엘|아모스|오바댜|요나|미가|나훔|하박국|스바냐|학개|스가랴|말라기|마태복음|마가복음|누가복음|요한복음|사도행전|로마서|고린도|갈라디아서|에베소서|빌립보서|골로새서|데살로니가|디모데|디도서|빌레몬서|히브리서|야고보서|베드로|요한일서|요한이서|요한삼서|유다서|요한계시록)',
            'bible_terms': r'(성경|말씀|구절|장|절|번역본|개역개정|NIV|KJV|ESV|NASB|통독|낭독|묵상)',
            'app_features': r'(검색|재생|복사|붙여넣기|다운로드|설정|메뉴|화면|동시|비교|나란히|병렬)',
            'technical_terms': r'(앱|어플|애플리케이션|업데이트|버전|기능|오류|에러|버그|설치|실행)'
        }
        ...
        # 일반 키워드 추출 (기존 방식)
        general_keywords = self.extract_keywords(text)
        keywords.extend(general_keywords)
        
        # 중복 제거 및 중요도 순 정렬
        unique_keywords = list(set(keywords))
        
        # 중요도 기반 정렬 (성경 관련 > 앱 기능 > 기술 용어 > 일반)
        priority_order = []
        for pattern_name in ['bible_books', 'bible_terms', 'app_features', 'technical_terms']:
            pattern = important_patterns[pattern_name]
            for keyword in unique_keywords:
                if re.search(pattern, keyword, re.IGNORECASE) and keyword not in priority_order:
                    priority_order.append(keyword)
        
        # 나머지 키워드 추가
        for keyword in unique_keywords:
            if keyword not in priority_order and len(keyword) >= 2:
                priority_order.append(keyword)
        
        return priority_order[:10]  # 상위 10개만 반환

    # ==================================================
    # 🔥 의미적 유사도 계산 메서드
    # ==================================================
    def calculate_semantic_similarity(self, query: str, ref_question: str, ref_answer: str) -> float:
        """질문과 참고 Q&A의 의미적 유사도 계산"""
        try:
            # 질문 간 유사도 (가중치 60%)
            question_similarity = self.calculate_text_similarity(query, ref_question)
            
            # 질문-답변 간 유사도 (가중치 40%) - 답변에서 질문 관련 내용 찾기
            answer_relevance = self.calculate_text_similarity(query, ref_answer)
            
            semantic_score = question_similarity * 0.6 + answer_relevance * 0.4
            
            return min(semantic_score, 1.0)
            
        except Exception as e:
            logging.error(f"의미적 유사도 계산 실패: {e}")
            return 0.0

    def calculate_text_similarity(self, text1: str, text2: str) -> float:
        """두 텍스트 간 단어 기반 유사도 계산"""
        if not text1 or not text2:
            return 0.0
        
        # 키워드 추출
        keywords1 = set(self.extract_enhanced_keywords(text1.lower()))
        keywords2 = set(self.extract_enhanced_keywords(text2.lower()))
    ...
    # 🔥 키워드 매칭 점수 계산 메서드
    # ==================================================
    def calculate_keyword_matching_score(self, query: str, ref_question: str, ref_answer: str) -> float:
        """키워드 매칭 기반 점수 계산"""
        try:
            query_keywords = set(self.extract_enhanced_keywords(query.lower()))
            question_keywords = set(self.extract_enhanced_keywords(ref_question.lower()))
            answer_keywords = set(self.extract_enhanced_keywords(ref_answer.lower()))
            
            if not query_keywords:
                return 0.0
            
            # 질문 키워드 매칭 (70%)
            q_intersection = len(query_keywords & question_keywords)
            q_score = q_intersection / len(query_keywords) if query_keywords else 0
            
            # 답변 키워드 매칭 (30%)
            a_intersection = len(query_keywords & answer_keywords)
            a_score = a_intersection / len(query_keywords) if query_keywords else 0
            ...
            logging.error(f"키워드 매칭 점수 계산 실패: {e}")
            return 0.0

    # ==================================================
    # 🔥 향상된 컨텍스트 품질 분석 메서드
    # ==================================================
    def analyze_context_quality_enhanced(self, similar_answers: list, query: str) -> dict:
        """향상된 컨텍스트 품질 분석"""
        ...
        if not similar_answers:
            return {
                'has_good_context': False,
                'best_score': 0.0,
                'recommended_approach': 'no_data_fallback',
                'context_summary': '관련 데이터가 없습니다.',
                'question_type': '일반문의',
                'context_relevance': 'none',
        ...
        # 점수 분포 분석
        scores = [ans['score'] for ans in similar_answers]
        ...
        # 🔥 핵심: 향상된 관련성 검증
        context_relevance = self.enhanced_context_relevance_check(
            question_analysis, similar_answers[:5], query
        )
        
        # 신뢰도 수준 계산
        confidence_level = self.calculate_confidence_level(
            best_score, avg_score, score_variance, context_relevance, 
            high_quality_count, len(similar_answers)
        )
        
        # 🔥 개선된 접근 방식 결정 로직
        approach = self.determine_enhanced_approach(
            best_score, context_relevance, confidence_level, 
            high_quality_count, medium_quality_count
        )
        
        analysis = {
            'has_good_context': context_relevance in ['high', 'medium'] and best_score >= 0.5,
            'best_score': best_score,
            'avg_score': avg_score,
            ...
            'context_summary': f"의도: {question_type}, 관련성: {context_relevance}, 신뢰도: {confidence_level}, 최고점수: {best_score:.3f}"
        }
        
        logging.info(f"향상된 컨텍스트 분석: {analysis}")
        return analysis

    # ==================================================
    # 🔥 향상된 관련성 검증 메서드
    # ==================================================
    def enhanced_context_relevance_check(self, question_analysis: dict, top_answers: list, query: str) -> str:
        """AI와 규칙 기반 검증을 결합한 관련성 검사"""
        
        try:
            # 1단계: AI 기반 관련성 검증 (기존 메서드 활용)
            ai_relevance = self.check_context_relevance_ai(question_analysis, 
                                                         [ans['category'] for ans in top_answers], 
                                                         query, top_answers)
            
            # 2단계: 규칙 기반 관련성 검증
            rule_relevance = self.rule_based_relevance_check(question_analysis, top_answers, query)
            
            # 3단계: 점수 기반 관련성 검증
            score_relevance = self.score_based_relevance_check(top_answers)
            
            # 4단계: 종합 판정 (가중 평균)
            relevance_scores = {
                'high': 3, 'medium': 2, 'low': 1, 'irrelevant': 0
            }
            
            ai_score = relevance_scores.get(ai_relevance, 0)
            rule_score = relevance_scores.get(rule_relevance, 0)
            score_score = relevance_scores.get(score_relevance, 0)
            
            # 가중 평균 (AI 50%, 규칙 30%, 점수 20%)
            weighted_score = (ai_score * 0.5 + rule_score * 0.3 + score_score * 0.2)
            
            # 최종 관련성 판정
            if weighted_score >= 2.5:
                final_relevance = 'high'
            elif weighted_score >= 1.8:
                final_relevance = 'medium'
            elif weighted_score >= 1.0:
                final_relevance = 'low'
            else:
                final_relevance = 'irrelevant'
            
            logging.info(f"향상된 관련성 검증: AI={ai_relevance}, 규칙={rule_relevance}, 점수={score_relevance} → 최종={final_relevance}")
            
            return final_relevance
            
        except Exception as e:
            logging.error(f"향상된 관련성 검증 실패: {e}")
            return 'medium'  # 안전한 기본값

    def rule_based_relevance_check(self, question_analysis: dict, top_answers: list, query: str) -> str:
        """규칙 기반 관련성 검증"""
        
        intent_type = question_analysis.get('intent_type', '')
        main_topic = question_analysis.get('main_topic', '')
        action_type = question_analysis.get('action_type', '')
        ...
        for answer in top_answers[:3]:
            answer_text = answer.get('answer', '').lower()
            question_text = answer.get('question', '').lower()
            
            # 답변에서 키워드 추출
            answer_keywords = set(self.extract_enhanced_keywords(answer_text))
            question_keywords = set(self.extract_enhanced_keywords(question_text))
            all_ref_keywords = answer_keywords | question_keywords
            
            # 키워드 일치도 계산
            keyword_overlap = len(query_keywords & all_ref_keywords)
            keyword_ratio = keyword_overlap / max(len(query_keywords), 1)
            
            # 의도별 특별 검증
            intent_match_score = self.check_intent_specific_match(
                intent_type, main_topic, action_type, answer_text, question_text
            )
            
            # 종합 점수 (키워드 60% + 의도 매칭 40%)
            total_score = keyword_ratio * 0.6 + intent_match_score * 0.4
            max_relevance_score = max(max_relevance_score, total_score)
        
        # 점수에 따른 관련성 판정
        if max_relevance_score >= 0.7:
            return 'high'
        elif max_relevance_score >= 0.5:
            return 'medium'
        elif max_relevance_score >= 0.3:
            return 'low'
        else:
            return 'irrelevant'

    def check_intent_specific_match(self, intent_type: str, main_topic: str, action_type: str, answer_text: str, question_text: str) -> float:
        """의도별 특별 매칭 검증"""
        
        # 의도별 키워드 패턴 정의
        intent_patterns = {
            '기능문의': {
                'keywords': ['기능', '사용법', '방법', '설정', '어떻게'],
                'weight': 0.8
            },
            '오탈자신고': {
                'keywords': ['오타', '오탈자', '틀렸', '잘못', '수정'],
                'weight': 0.9
            },
            '기술지원': {
                'keywords': ['오류', '에러', '안됨', '안되', '문제', '버그'],
                'weight': 0.8
            },
            '개선제안': {
                'keywords': ['개선', '제안', '추가', '업데이트', '기능'],
                'weight': 0.7
            }
        }
        ...
        # 주제별 키워드 패턴
        topic_patterns = {
            '성경본문': ['성경', '말씀', '구절', '장', '절', '번역본'],
            '음원재생': ['재생', '듣기', '음성', '소리', '연속'],
            '검색기능': ['검색', '찾기', '찾아서'],
            '텍스트복사': ['복사', '붙여넣기', '텍스트', '워드'],
            '앱기능': ['앱', '기능', '설정', '메뉴']
        }
        
        combined_ref_text = (answer_text + ' ' + question_text).lower()
        
        # 의도 매칭 점수
        intent_score = 0
        if intent_type in intent_patterns:
            pattern = intent_patterns[intent_type]
            matches = sum(1 for keyword in pattern['keywords'] if keyword in combined_ref_text)
            intent_score = (matches / len(pattern['keywords'])) * pattern['weight']
        
        # 주제 매칭 점수
        topic_score = 0
        if main_topic in topic_patterns:
            keywords = topic_patterns[main_topic]
            matches = sum(1 for keyword in keywords if keyword in combined_ref_text)
            topic_score = matches / len(keywords)
        
        # 종합 점수
        return (intent_score * 0.6 + topic_score * 0.4)

    def score_based_relevance_check(self, top_answers: list) -> str:
        """점수 기반 관련성 검증"""
        
        if not top_answers:
            return 'irrelevant'
        
        best_score = top_answers[0]['score']
    ...
    # 🔥 신뢰도 수준 계산 메서드
    # ==================================================
    def calculate_confidence_level(self, best_score: float, avg_score: float, 
                                  score_variance: float, context_relevance: str,
                                  high_quality_count: int, total_count: int) -> str:
        """답변 생성 신뢰도 수준 계산"""
        
        # 각 요소별 점수 계산
        score_confidence = min(best_score * 1.2, 1.0)  # 최고 점수 기반
        consistency_confidence = 1.0 - min(score_variance, 0.5) * 2  # 점수 일관성
        relevance_confidence = {'high': 1.0, 'medium': 0.7, 'low': 0.4, 'irrelevant': 0.1}.get(context_relevance, 0.1)
        quality_ratio = high_quality_count / max(total_count, 1)
    ...
    # 🔥 향상된 접근 방식 결정 메서드
    # ==================================================
    def determine_enhanced_approach(self, best_score: float, context_relevance: str, 
                                   confidence_level: str, high_quality_count: int, 
                                   medium_quality_count: int) -> str:
        """개선된 접근 방식 결정 로직"""
        
        # 🔥 핵심: 관련성과 신뢰도를 종합적으로 고려
        
        # 1. 데이터 부족 상황
        if context_relevance == 'irrelevant' or confidence_level == 'very_low':
            return 'no_data_fallback'
        
        # 2. 고품질 직접 사용 조건 (매우 엄격)
        if (best_score >= 0.9 and context_relevance == 'high' and 
            confidence_level in ['high', 'medium'] and high_quality_count >= 1):
            return 'direct_use'
        
        # 3. 강한 컨텍스트 GPT 생성 조건
        if (best_score >= 0.7 and context_relevance in ['high', 'medium'] and 
            confidence_level in ['high', 'medium']):
            return 'gpt_with_strong_context'
        
        # 4. 약한 컨텍스트 GPT 생성 조건
        if (best_score >= 0.5 and context_relevance in ['high', 'medium', 'low'] and
            (high_quality_count + medium_quality_count) >= 2):
            return 'gpt_with_weak_context'
        
        # 5. 기본 폴백 (참고답변 활용)
        if context_relevance in ['medium', 'low'] and best_score >= 0.4:
    ...
    # ==================================================
    # 🔥 향상된 GPT 프롬프트 생성 메서드 (기존 메서드 오버라이드)
    # ==================================================
    def get_gpt_prompts(self, query: str, context: str, lang: str = 'ko') -> tuple:
        """향상된 GPT 프롬프트 생성 (정확도 개선 버전)"""
        
        # 기본 컨텍스트 분석 생성
        context_analysis = {
            'recommended_approach': 'gpt_with_strong_context',
        ...
        context_relevance = context_analysis.get('context_relevance', 'medium')
        question_analysis = context_analysis.get('question_analysis', {})
        
        if lang == 'en':
            system_prompt = f"""You are a GOODTV Bible App customer service expert with 15+ years of experience.

🎯 CRITICAL MISSION: Provide accurate, helpful answers based on ACTUAL app capabilities.

📋 Context Analysis:
- Relevance: {context_relevance}
- Confidence: {confidence_level}
- Customer Intent: {question_analysis.get('intent_type', 'N/A')}
- Main Topic: {question_analysis.get('main_topic', 'N/A')}

⚠️ ABSOLUTE REQUIREMENTS:
1. **NEVER claim features don't exist without clear evidence**
2. **ALWAYS prioritize reference answers over assumptions**
3. **If reference answers suggest a feature exists, provide that guidance**
4. **Only say "not supported" if reference clearly states limitations**

🔍 Reference Answer Strategy:
- High relevance ({context_relevance}): Follow reference answers closely
- Include specific steps if provided in references
- Adapt reference solutions to customer's exact situation
- Maintain the helpful, solution-oriented tone

✅ Response Guidelines:
- Address customer as "Dear user"
- Use "GOODTV Bible App" or "Bible App"
- Provide step-by-step guidance when possible
- If uncertain, say "Let me check this for you" instead of "not supported"

🚫 Formatting Rules:
- NO greetings ("Hello", "Thank you")
- NO closings ("Best regards", "God bless")
- Write ONLY the main helpful content
- Use natural sentences, not HTML tags"""

            user_prompt = f"""Customer Question: {query}

Reference Answers (Confidence: {confidence_level}):
{context}

IMPORTANT: The reference answers above contain actual information about app capabilities. 
Base your response on these references. If they indicate a feature exists, provide guidance accordingly.

Write a helpful, accurate response focused on solving the customer's specific need."""

        else:  # 한국어
            system_prompt = f"""당신은 15년 경력의 GOODTV 바이블 애플 고객지원 전문가입니다.

🎯 핵심 임무: 앱의 실제 기능을 바탕으로 정확하고 도움이 되는 답변 제공

📋 컨텍스트 분석:
- 관련성: {context_relevance}
- 신뢰도: {confidence_level}
- 고객 의도: {question_analysis.get('intent_type', 'N/A')}
- 주요 주제: {question_analysis.get('main_topic', 'N/A')}

⚠️ 절대 준수사항:
1. **명확한 근거 없이 기능이 없다고 단정하지 말 것**
2. **참고답변을 추측보다 우선시할 것**
3. **참고답변에서 기능이 있다고 하면 해당 안내를 제공할 것**
4. **참고답변에서 명확히 제한사항을 언급할 때만 "지원되지 않음" 표현**

🔍 참고답변 활용 전략 (관련성: {context_relevance}):
- 높은 관련성: 참고답변의 해결방법을 충실히 따를 것
- 구체적 단계가 있으면 포함할 것
- 고객의 정확한 상황에 맞게 조정할 것
- 도움이 되고 해결지향적인 톤 유지

✅ 답변 작성 가이드:
- "성도님"으로 호칭
- "바이블 애플" 또는 "GOODTV 바이블 애플" 사용
- 가능하면 단계별 안내 제공
- 불확실하면 "지원되지 않음"보다 "확인해드리겠습니다" 표현

🚫 형식 제한:
- 인사말 금지 ("안녕하세요", "감사합니다")
- 끝맺음말 금지 ("평안하세요", "주님 안에서")
- 핵심 도움 내용만 작성
- HTML 태그 사용하지 말고 자연스러운 문장으로"""

            user_prompt = f"""고객 문의: {query}

참고답변들 (신뢰도: {confidence_level}):
{context}

중요: 위 참고답변들은 앱의 실제 기능에 대한 정보를 담고 있습니다.
이 참고답변을 기반으로 응답하세요. 기능이 존재한다고 나와있으면 해당 안내를 제공하세요.

고객의 구체적인 요청을 해결하는 데 도움이 되는 정확한 답변을 작성하세요."""
        
        return system_prompt, user_prompt

    # ==================================================
    # 🔥 기존 메서드들을 향상된 버전으로 오버라이드
    # ==================================================
    
    # 기존 search_similar_answers 메서드를 향상된 버전으로 교체
    def search_similar_answers(self, query: str, top_k: int = 5, similarity_threshold: float = 0.7, lang: str = 'ko') -> list:
        """향상된 검색으로 오버라이드"""
        return self.search_similar_answers_enhanced(query, top_k, similarity_threshold, lang)

    # 기존 analyze_context_quality 메서드를 향상된 버전으로 교체  
    def analyze_context_quality(self, similar_answers: list, query: str) -> dict:
        """향상된 컨텍스트 분석으로 오버라이드"""
        return self.analyze_context_quality_enhanced(similar_answers, query)
